...
//    if(data()->initVersion<30) 
   { 
    ...
    if(data()->initVersion<39) {
        // Release orientation lock for all existing users.
        // Values 0 (landscape) and 8 (reverseLandscape) were the only defaults
        // ever written by the app.  Reset both to the UNSPECIFIED sentinel (127)
        // so the OS and user control rotation freely.
        if(data()->orientation == 0 || data()->orientation == 8) {
            data()->orientation = 127u;
        }
    }
                                ...
                                if(data()->initVersion<6) {
                                   if(data()->initVersion<4) {
                                      if(data()->varcount==0) {
                                        ...
                                        data()->orientation=1;
                          #else
                                        // New installs: free rotation by default.
                                        data()->orientation=127u; // ORIENTATION_UNSPECIFIED_STORED
                          #endif
